...
class EmbeddingClient:
    ...
    def embed_and_store(self, texts: list[str], collection_name: str = "default_collection"):
        ...
        for i, text in enumerate(texts):
            embedding = self.ollama_client.embed(model="mxbai-embed-large")
            collection.add(
                ids=[str(i)],
    ...
    def search(self, query: str, collection_name: str = "default_collection", n_results: int = 5) -> list[str]:
        """
        Searches for similar texts in a Chroma collection.
        ...
        embedding = self.ollama_client.embed(model="mxbai-embed-large", prompt=query)
...
if __name__ == "__main__":
    ...
    # search_results = client.search("test")
